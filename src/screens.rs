...
pub fn SessionScreen(
    ...
    rsx! {
        div { class: "screen screen-session right-rail",
            ...
            WidgetSection { title: "Session detail".to_string(), expanded: session_detail_expanded, body: rsx! {
            ...
            WidgetSection { title: "Provider detail".to_string(), expanded: provider_expanded, body: rsx! {
                    ...
                    div { class: "kv-grid widget-kv-grid",
                        for provider in &data.providers {
                            div { class: "kv-row",
                                ...
                                span { class: "kv-value", "{provider_inventory_value(provider)}" }
                            }
                        }
                    }
            ...
            WidgetSection { title: "Control-plane detail".to_string(), expanded: control_plane_expanded, body: rsx! {
            ...
            WidgetSection { title: "Auspex shell".to_string(), expanded: shell_expanded, body: rsx! {
                div { class: "kv-grid widget-kv-grid",
            ...
            if expanded {
                ...
                div { class: "widget-disclosure",
                    button {
                        ...
                        r#type: "button",
                        "aria-expanded": if collapsed_open { "true" } else { "false" },
                        h2 { class: "screen-section-title widget-section-title", "{title}" }
                    }
                    if collapsed_open {
...
fn WidgetSection(title: String, expanded: bool, body: Element) -> Element {
    ...
    if expanded {
        render_widget_section_static(&title, true, true, body)
    } else {
        let widget_id = title.to_ascii_lowercase().replace(' ', "-");
        rsx! {
            section {
                class: "screen-section widget-section widget-section-collapsible",
                "data-widget": widget_id,
                "data-surface": "panel",
                "data-elevation": "1",
                div { class: "widget-disclosure",
                    ...
                    if is_open {
                        div { class: "widget-disclosure-body", {body_for_click} }
                    }
                }
            }
        }
...
fn render_widget_section(title: &str, expanded: bool, body: Element) -> Element {
    if expanded {
        ...
        render_widget_section_static(title, false, false, body)
    }
}
...
mod tests {
    ...
    fn session_detail_widget_titles_reflect_contextual_role() {
        ...
        assert!(shell_debug.contains("widget-disclosure"));
        assert!(shell_debug.contains("aria-expanded"));
    ...
    fn instance_descriptor_render_includes_capabilities() {
        ...
        assert_eq!(
            capability_badge_class("shutdown"),
            "badge badge-blocked capability-badge"
        );
